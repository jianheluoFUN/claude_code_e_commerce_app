...
// This is the dashboard page component for store owner
// The routing is "/dashboard"
export default async function DashboardPage() {
    ...
    // Check if user has a store from "stores" table based on owner_id
    const {
        data: store
    } = await supabase
        .from("stores")
    ...
    if (!store) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
                ...
                {/* Link to "/dashboard/settings" routing */}
    ...
    if (store.status === "pending") {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
                ...
                <p className="text-muted-foreground mb-6 max-w-md">
                    Your store is currently under review. You&apos;ll be able to add products and receive orders once
                    approved.
                </p>
    ...
    // Fetch dashboard stats from "products" table based on "store_id"
    const {count: productCount} = await supabase
        .from("products")
        .select("*", {count: "exact", head: true})
        .eq("store_id", store.id)

    // retrieve from "orders" table based on "store_id"
    ...
    // retrieve first 5 orders for the store
